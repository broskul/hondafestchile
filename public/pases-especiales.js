(() => {
  ...
  function renderLevelBenefits(level) {
    const benefits = Array.isArray(level.benefits) ? level.benefits : [];
    return `
      <div class="pass-refreshment-row">
        <span class="generic-drink-icon" aria-hidden="true"></span>
        <div>${benefits.map((benefit) => `<span>${escapeHtml(benefit)}</span>`).join("")}</div>
      </div>`;
  }
      ...
      .map(
        (level) => `
          <article class="pass-level-card accent-${escapeHtml(level.accent)} ${level.featured ? "featured" : ""}" data-level-card="${escapeHtml(level.id)}">
            ...
            <p>Upgrade para una entrada HFC 2026. Cada Pistón suma un boleto al sorteo.</p>
            ${renderLevelBenefits(level)}
            <span class="pass-level-net">${HFC.formatCurrency(level.netPrice)} neto</span>
            <strong class="pass-level-price">${HFC.formatCurrency(level.price)}</strong>
            <small>Final con IVA y cargo de servicio de 8%</small>
            <button class="button ${level.featured ? "primary" : "secondary"} full" type="button" data-select-level="${escapeHtml(level.id)}" ${purchaseAvailable ? "" : "disabled"}>${purchaseAvailable ? "Elegir upgrade" : "Próximamente"}</button>
          </article>`
      )
      .join("");
  ...
  function selectLevel(levelId) {
    ...
    HFC.$("#selectedPassSummary").innerHTML = `
      ...
      <p>${HFC.formatCurrency(selectedLevel.netPrice)} neto. Total con IVA y cargo de servicio de 8%.</p>
      ${renderLevelBenefits(selectedLevel)}
  ...
  async function mountCheckout() {
    ...
    if (!catalog.active) {
      ...
      HFC.setStatus(status, catalog.unavailableMessage || "Los Pases de Pistones estarán disponibles próximamente.");
      return;
    }
    form.addEventListener("submit", async (event) => {
      event.preventDefault();
      if (!selectedLevel) return HFC.setStatus(status, "Selecciona un nivel de Pase de Pistones.", true);
      ...
      HFC.setStatus(status, "Creando tu Pase de Pistones...");
      try {
