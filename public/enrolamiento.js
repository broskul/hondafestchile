(() => {
  ...
  function profileForm(item, mode) {
    ...
    return `
      ...
      <form class="profile-completion-form enrollment-form" data-enrollment-form data-order-id="${escapeHtml(order.id || "")}">
        ...
        <button class="button primary full" type="submit">${isSpecialPass ? "Emitir Pase Especial" : "Emitir entradas"}</button>
      </form>
    `;
  }
  ...
  function renderCompleted(data) {
    ...
    content().innerHTML = `
      ...
      <h2>${isSpecialPass ? "Pase Especial emitido" : "Entradas emitidas"}</h2>
          ...
          .map(
            (item) => `
              <article class="ticket-pass">
                ...
                ${isSpecialPass ? `<strong>${escapeHtml(item.pistonCount)} ${Number(item.pistonCount) === 1 ? "Pistón" : "Pistones"}</strong><small>NO ES VÁLIDO COMO ENTRADA</small>` : ""}
              </article>
            `
          )
          .join("")}
  ...
  function mountEnrollmentForm(item, mode) {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      HFC.setStatus(status(), `Guardando datos y emitiendo ${item.order?.kind === "special_pass" ? "el Pase Especial" : "entradas"}...`);
