(() => {
  ...
  function render(data) {
    ...
    node.innerHTML = `
      ...
      ${Array.isArray(pass.benefits) && pass.benefits.length ? `<ul class="order-items">${pass.benefits.map((benefit) => `<li>${escapeHtml(benefit)}</li>`).join("")}</ul>` : ""}
  ...
  async function lookup(code = currentCode) {
    ...
    node.textContent = "Consultando Pase de Pistones...";
    try {
  ...
  async function operate(action) {
    ...
    try {
      ...
      render(data);
      HFC.toast(data.changed ? "Estado del Pase de Pistones actualizado." : "El estado ya estaba registrado.");
    } catch (error) {
