...
process.env.SPECIAL_PASSES_ENABLED = "true";
...
async function main() {
  ...
  try {
    ...
    assert.equal(catalog.notEntryLabel, "UPGRADE: REQUIERE UNA ENTRADA HFC 2026");
    ...
    assert.equal(created.order.total, 19278);
    ...
    const ticketOrder = (
      await request(base, "/api/orders/from-cart", {
        method: "POST",
        body: JSON.stringify({
          email: "qa.entrada@example.com",
          name: "QA Entrada HFC",
          rut: "11.111.111-1",
          phone: "+56 9 5555 5678",
          termsAccepted: true,
          items: [{ eventId: "honda-fest-chile-2026", ticketTypeId: "hfc-2026-galeria", quantity: 1 }]
        })
      })
    ).data;
    assert.equal(ticketOrder.order.total, 8996);
    const paidTicketOrder = (
      await request(base, `/api/orders/${encodeURIComponent(ticketOrder.order.id)}/simulate-payment`, {
        method: "POST",
        body: "{}"
      })
    ).data;
    assert.equal(paidTicketOrder.tickets.length, 1);
    assert.equal(paidTicketOrder.tickets[0].pistonCount, 1);
    assert.equal(paidTicketOrder.tickets[0].drawEntryCodes.length, 1);

    const summary = (
      await request(base, "/api/backoffice/summary", { headers: operatorHeaders })
    ).data;
    assert.equal(summary.summary.specialPasses, 1);
    assert.equal(summary.summary.pistonsIssued, 6);
    ...
    console.log("Pases de Pistones: flujo de compra, enrolamiento, emisión, retiro, acceso y exportación verificado.");
  } finally {
